...
def outlier_scalar_metric_wrapper(preds: np.ndarray, gt: np.ndarray,
                                  score: np.ndarray = None, cfg: DictConfig = None,
    ...
    if score is not None:
        ...
        metrics["f1"], metrics["adj_f1_threshold"] = (
...
def subjectwise_outlier_metric_wrapper(preds: np.ndarray, gt: np.ndarray, score: np.ndarray,
                                       cfg: DictConfig = None, global_metrics: dict = None):
    ...
    for subj_idx in range(no_subjects):
        ...
        if score is not None:
            ...
            score_subject = None
        metrics_subject = outlier_scalar_metric_wrapper(preds=pred_subject, gt=gt_subject, score=score_subject, cfg=cfg,
                                                        adj_f1_threshold=adj_f1_threshold)
        if subj_idx == 0:
...
def update_CI_to_outlier_scalars(metrics_subjectwise, global_metrics, p=0.05):
    ...
    for key in metrics_subjectwise.keys():
        ...
        if np.all(array_of_values == None):
            ci = None
        else:
...
def get_scalar_outlier_metrics(preds: np.ndarray,
                               gt: np.ndarray,
                               score: np.ndarray,
                               cfg: DictConfig = None,
    ...
    metrics = {}
    metrics['global'] = outlier_scalar_metric_wrapper(preds=preds, gt=gt, score=score, cfg=cfg,
                                                      adj_f1_threshold=threshold)
    metrics['subjectwise'] = subjectwise_outlier_metric_wrapper(preds=preds, gt=gt, score=score, cfg=cfg,
                                                                global_metrics=metrics['global'])
    metrics['global'] = update_CI_to_outlier_scalars(metrics_subjectwise=metrics['subjectwise'],
                                                     global_metrics=metrics['global'])
...
def metrics_per_split(
    ...
    # The vanilla metrics
    metrics_test["scalars"] = get_scalar_outlier_metrics(preds=pred, gt=gt, score=energy, cfg=None, threshold=None)
